...
static int dsi_panel_reset(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(panel->reset_config.lcd_mode_sel_gpio))
	{
		...
		if ((panel->reset_config.mode_sel_state == MODE_SEL_DUAL_PORT) || (panel->reset_config.mode_sel_state == MODE_GPIO_LOW))
			out = false;
		else if ((panel->reset_config.mode_sel_state == MODE_SEL_SINGLE_PORT) ||
				 (panel->reset_config.mode_sel_state == MODE_GPIO_HIGH))
			out = true;
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if (panel->bl_config.bl_remap_flag && panel->bl_config.brightness_max_level && panel->bl_config.bl_max_level)
	{
...
int dsi_panel_get_mode_count(struct dsi_panel *panel)
{
	...
	num_dfps_rates = !panel->dfps_caps.dfps_support ? 1 : panel->dfps_caps.dfps_list_len;

	num_bit_clks = !panel->dyn_clk_caps.dyn_clk_support ? 1 : panel->dyn_clk_caps.bit_clk_list_len;
...
static int panel_disp_param_send_lock(struct dsi_panel *panel, int param)
{
	...
	if (!panel->panel_initialized && (param & 0x0F000000) != DISPPARAM_FOD_BACKLIGHT_ON && (param & 0x0F000000) != DISPPARAM_FOD_BACKLIGHT_OFF && (param & 0x0F0000) != DISPPARAN_FOD_FLAG)
	{
		pr_err("[LCD] panel not ready!\n");
		mutex_unlock(&panel->panel_lock);
		return rc;
	...
	case DISPPARAM_DIMMING:
		...
		{
			if (ktime_after(ktime_get(), panel->fod_hbm_off_time) && ktime_after(ktime_get(), panel->fod_backlight_off_time))
			{
	...
	case DISPPARAM_ELVSS_DIMMING_ON:
		...
		((u8 *)panel->cur_mode->priv_info->cmd_sets[DSI_CMD_SET_DISP_ELVSS_DIMMING_OFF].cmds[2].msg.tx_buf)[1] = (panel->elvss_dimming_cmds.rbuf[0]);
	...
	case DISPPARAM_ELVSS_DIMMING_OFF:
		...
		((u8 *)panel->cur_mode->priv_info->cmd_sets[DSI_CMD_SET_DISP_ELVSS_DIMMING_OFF].cmds[2].msg.tx_buf)[1] = (panel->elvss_dimming_cmds.rbuf[0]) & 0x7F;
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_DISP_ELVSS_DIMMING_OFF);
		break;
	default:
		break;
